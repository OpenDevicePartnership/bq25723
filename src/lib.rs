//! This is a platform-agnostic Rust driver for the Texas Instruments BQ25723 Battery
...
//!
//! [`Datasheet`]: https://www.ti.com/lit/ds/symlink/bq25723.pdf
...
/// BQ25723 errors
pub enum BQ25723Error<I2cError> {
    Bus(I2cError),
}
...
/// BQ25723 interface, which takes an async I2C bus
...
impl<I2c: embedded_hal_async::i2c::I2c> device_driver::AsyncRegisterInterface for DeviceInterface<I2c> {
    ...
    async fn write_register(
        ...
        // Because the BQ25723 has a mix of 1 byte and 2 byte registers that can be written to,
...
impl<E: embedded_hal_async::i2c::Error> charger::Error for BQ25723Error<E> {
...
pub struct Bq25723<I2c: embedded_hal_async::i2c::I2c> {
    pub device: Device<DeviceInterface<I2c>>,
}

impl<I2c: embedded_hal_async::i2c::I2c> Bq25723<I2c> {
...
impl<I2c: embedded_hal_async::i2c::I2c> charger::ErrorType for Bq25723<I2c> {
    type Error = BQ25723Error<I2c::Error>;
}

impl<I2c: embedded_hal_async::i2c::I2c> charger::Charger for Bq25723<I2c> {
    async fn charging_current(&mut self, current: charger::MilliAmps) -> Result<charger::MilliAmps, Self::Error> {
        let reg_value = Self::ma_to_reg_value(current);
        self.device
            .charge_current()
            .write_async(|w| w.set_charge_current(reg_value))
            .await?;
        Ok(Self::reg_value_to_ma(
            self.device.charge_current().read_async().await?.charge_current(),
        ))
    }

    async fn charging_voltage(&mut self, voltage: charger::MilliVolts) -> Result<charger::MilliVolts, Self::Error> {
        let reg_value = Self::mv_to_reg_value(voltage);
        self.device
            .charge_voltage()
            .write_async(|w| w.set_charge_voltage(reg_value))
            .await?;
        Ok(Self::reg_value_to_mv(
            self.device.charge_voltage().read_async().await?.charge_voltage(),
        ))
    }
}
...
mod tests {
    ...
    async fn charging_current_trait_test() {
        ...
        let i2c = Mock::new(&expectations);
        let mut bq = Bq25723::new(i2c);

        let charge_current = bq.charging_current(1984).await.unwrap();
